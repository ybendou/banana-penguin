...
def openimg(cl,title='', sample=None, dataset='train', filepath='', directory=''):
    ...
    if type(cl)==int:
        if sample==None:
            ...
            idx = sample
        filename=src[idx+600*cl][0]
        im = Image.open(directory +filename)
        plt.title(title)
        plt.imshow(np.array(im))

def getimg(cl,title='', sample=None, filepath='', directory=''):
    src = opencsv(filepath)
...
def compute_hyperplans_and_grid(feats, ARGS, crops=False):
    ...
    feats = feats[:, :ARGS.n_shots]
    if crops: feats = torch.mean(feats, dim=2)
    
    grid_size=100
    x_min, x_max = feats.reshape(-1, ARGS.n_ways-1).min(dim=0)[0][0].item(), feats.reshape(-1, ARGS.n_ways-1).max(dim=0)[0][0].item()
    y_min, y_max = feats.reshape(-1, ARGS.n_ways-1).min(dim=0)[0][1].item(), feats.reshape(-1, ARGS.n_ways-1).max(dim=0)[0][1].item()
    ...
    hyperplans = []
    for n in range(-1, ARGS.n_ways-1):
...
def visualize_few_shot_space(reduced_features, run_classes, run_indices, ARGS, query_start=0, query_end=2, plot_support=True, crops=False, figsize=(25, 15)):
    """
        ...
        - In:
            * feats: Features Tensor of dimension [ARGS.n_ways, ARGS.n_shots+ARGS.n_queries, 2].
    ...
    assert query_start<ARGS.n_queries, f'query_start should be smaller than {ARGS.n_queries}'
    assert query_start>=0, f'query_start should be larger than 0'
    assert query_end<=ARGS.n_queries, f'query_end should be smaller than {ARGS.n_queries}'
    ...
    for c in range(reduced_features.shape[0]): 
        support = reduced_features[c, :ARGS.n_shots]
        support_indices = [i.item() for i in run_indices[c][:ARGS.n_shots]]
        if plot_support:
    ...
    # Queries 
    for c in range(reduced_features.shape[0]):
        query = reduced_features[c, ARGS.n_shots:]
        query_indices = [i.item() for i in run_indices[c][ARGS.n_shots:]]
        for q in range(min(query.shape[0], query_start),min(query.shape[0], query_end)):
            query_mean = query[q].mean(dim=0)
            plt.scatter(query[q][:, 0].cpu(), query[q][: , 1].cpu(), label=f'C={run_classes[c]} | s={query_indices[q]}[query]', color=colors[c], edgecolor=colors[c], s=100, marker=markers[ARGS.n_shots+q-query_start])
            plt.scatter(query_mean[0].cpu(), query_mean[1].cpu(), color=colors[c], s=300, marker=markers[ARGS.n_shots+q-query_start], linewidth=4)
...
def hover_few_shot_space(reduced_features, run_classes, run_indices, ARGS, figsize=(25, 15), query_start=0, query_end=2, plot_support=True, crops=False, images_path='../data/images/images_crops/'):
    """
        Visualize the feature space. Allows to plot multiple queries at the same time. 
        Plotting all queries simultaneously is not recommended as it would overload the plot.
        - In:
    ...
    for c in range(reduced_features.shape[0]): 
        ...
        for q in range(min(query.shape[0], query_start),min(query.shape[0], query_end)):
            ...
            if crops : 
                c_ind, q_ind = c, ARGS.n_shots+q
            else:
                c_ind, q_ind = run_classes[c], run_indices[c][q+ARGS.n_shots]
